...
function admin_no_clip()
	...
	if noclip then -- activé
		...
		Notify("Noclip ~g~Faal Shod")
	else -- désactivé
		SetEntityVisible(ped, true, false)
		Notify("Noclip ~r~Gheyre Faal Shod")
	end
end
...
Citizen.CreateThread(function()
	...
	WarMenu.CreateMenu('MainMenu', '~r~Red ~g~Life')
	WarMenu.SetSubTitle('MainMenu', 'Staff Menu Persian Codes Configed By ArDaVaN81')
...
-- Véhicule Menu
		...
		elseif WarMenu.IsMenuOpened('VehMenu') then
			...
			elseif WarMenu.CheckBox('Auto Dv', deleteVeh, function(enabled)
					deleteVeh = enabled
				end) then
...
-- Menu autre joueur
		...
		elseif WarMenu.IsMenuOpened("OnlinePlayerMenu") then
			for _, i in ipairs(GetActivePlayers()) do
				if GetPlayerServerId(i) ~= 0 and WarMenu.MenuButton("~p~["..GetPlayerServerId(i).."]~w~ "..GetPlayerName(i).." "..(IsPedDeadOrDying(GetPlayerPed(i), 1) and "~r~[Morde]" or "~g~[Zende]"), 'PlayerOptionsMenu') then
					SelectedPlayer = i
				end
			end
		...
		elseif WarMenu.IsMenuOpened("PlayerOptionsMenu") then
			...
			if WarMenu.Button('Spect Kardan', (Spectating and "~g~[SPECTATING]")) then
				SpectatePlayer(SelectedPlayer)
				name = GetPlayerName(SelectedPlayer)
				TriggerServerEvent("logMenuAdmin", '\nSpec un joueur ('..name..')')
			elseif WarMenu.Button('Teleport Kardan') then
				local Entity = IsPedInAnyVehicle(PlayerPedId(), false) and GetVehiclePedIsUsing(PlayerPedId()) or PlayerPedId()
				SetEntityCoords(Entity, GetEntityCoords(GetPlayerPed(SelectedPlayer)), 0.0, 0.0, 0.0, false)
			elseif WarMenu.MenuButton('Aslahe Dadan', 'SingleWepPlayer') then
			elseif WarMenu.Button('Mashin Dadan') then
			...
			elseif WarMenu.Button("Az Mashin Piade Kardan") then
				ClearPedTasksImmediately(GetPlayerPed(SelectedPlayer))
			elseif WarMenu.Button("Revive Kardan ~g~[Dar Hale RP]") then
		...
		elseif WarMenu.IsMenuOpened("SingleWepPlayer") then
			...
			WarMenu.Display()
		elseif IsDisabledControlPressed(0, 213) then
			ESX.TriggerServerCallback('RubyMenu:getUsergroup', function(group)
				...
				if playergroup == 'superadmin' or playergroup == 'owner' then
					...
					ESX.ShowAdvancedNotification('STAFF INFO', 'STAFF MENU ~g~ON', 'Menu Admini Baz Shod.\nRank Admini:~g~ '..playergroup..'', 'CHAR_DEVIN', 8)
				elseif playergroup == 'dev' or playergroup == 'mod' or playergroup == 'admin' then
					superadmin = false
					Citizen.Wait(10)
					WarMenu.OpenMenu('MainMenu')
					ESX.ShowAdvancedNotification('STAFF INFO', 'STAFF MENU ~g~ON', 'Menu Admini Baz Shpd.\nRank Admini:~g~ '..playergroup..'', 'CHAR_DEVIN', 8)
				end
			end)
		end
